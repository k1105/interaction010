...
export const HandSketch = ({ handpose }: Props) => {
  ...
  const draw = (p5: p5Types) => {
    ...
    if (hands.left.length > 0) {
      p5.push();
      const a = hands.left[5];
      const b = {
        x: hands.left[13].x - hands.left[5].x,
        y: hands.left[13].y - hands.left[5].y,
      };
    ...
    if (hands.right.length > 0) {
      p5.push();
      const a = hands.right[5];
      const b = {
        x: hands.right[13].x - hands.right[5].x,
        y: hands.right[13].y - hands.right[5].y,
      };
      if (a.x * b.y - a.y * b.x < 0) {
        p5.fill(255, 0, 0);
      } else {
